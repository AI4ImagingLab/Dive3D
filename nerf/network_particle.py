    ...
    def __init__(self, 
        ...
        if self.opt.bg_radius > 0:
            ...
            if not self.opt.complex_bg:
                print('222-2')
                self.encoder_bgs = torch.nn.ModuleList([get_encoder('frequency_torch', input_dim=3, multires=4)[0] for _ in range(self.n_particles)])
            else:
                print('222-3')
                self.encoder_bgs = torch.nn.ModuleList([get_encoder('frequency_torch', input_dim=3, log2_hashmap_size=19, desired_resolution=self.opt.desired_resolution * self.bound, interpolation='smoothstep')[0] for _ in range(self.n_particles)])
            print('222-4')
